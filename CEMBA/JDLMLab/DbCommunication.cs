...
namespace JDLMLab
{
    class DbCommunication
    {
        ...
        private void vytvorTabulky()
        {
            ...
            sql += 
                "CREATE TABLE `merania` (`id` int(11) NOT NULL AUTO_INCREMENT,"+
                "`x` double NOT NULL,`y_id` int(11) NOT NULL,`Intensity` bigint(11) unsigned NOT NULL,"+
                "`Current` double NOT NULL,`Capillar_pressure` double NOT NULL,"+
                "`Chamber_pressure` double NOT NULL,`Temperature` double NOT NULL,"+
                "`x_num` int(11) NOT NULL,PRIMARY KEY(`id`),KEY `id_y` (`y_id`),CONSTRAINT `obmedzenie2` FOREIGN KEY (`y_id`) REFERENCES `rows` (`id`) ON DELETE CASCADE ON UPDATE CASCADE) ENGINE = InnoDB AUTO_INCREMENT = 90 DEFAULT CHARSET = latin1;";
        ...
        public DataSet meranie(int headerId, int cycleNum = 0)
        {
            ...
            sql += " order by r.y asc,m.x_num asc,r.cycle_num asc ";
        ...
        public DataSet meraniePreGraf(int headerId)
        {
            ...
            sql += " order by cyklus asc,m.x_num asc";
            return getDataSet(sql);
        }
        ...
        public DataSet meranieAvg(int headerId)
        {
            ...
            string sql = "select round(avg(m.x),2) as " + xAlias + ",r.y as " + yAlias + ",avg(m.Intensity) as Intensity,avg(m.Current) as Current,avg(m.Capillar_pressure) as 'Capillar pressure',avg(m.Chamber_pressure) as 'Chamber pressure',avg(m.Temperature) as Temperature from merania m left join rows r on r.id = m.y_id where header_id = " + headerId;
            sql += " group by m.x_num order by r.y asc,m.x_num asc";
        ...
        public DataSet meranieSum(int headerId)
        {
            ...
            string sql = "select round(avg(m.x),2) as "+ xAlias + ",r.y as " + yAlias + ",sum(m.Intensity) as Intensity,avg(m.Current) as Current,avg(m.Capillar_pressure) as 'Capillar pressure',avg(m.Chamber_pressure) as 'Chamber pressure',avg(m.Temperature) as Temperature from merania m left join rows r on r.id = m.y_id where header_id = " + headerId;
            sql += " group by m.x_num order by r.y asc,m.x_num asc";
        ...
        public void addKroky(List<KrokMerania> ks,int cyklus=1)
        {
            ...
            {
                sql.Append("insert into merania (x,y_id,Intensity,Current,Capillar_pressure,Chamber_pressure,Temperature,x_num) " +
                "values(@x" +
                krokNum.ToString() + ",@y_id" + krokNum.ToString() + ",@sig" +
                krokNum.ToString() + ",@current" + krokNum.ToString() + ",@kapillar" +
                krokNum.ToString() + ",@chamber" + krokNum.ToString() + ",@temperature" +
                krokNum.ToString() + ",@x_num" + krokNum.ToString() + ");");
                krokNum++;
            }
            MySqlCommand c = new MySqlCommand(sql.ToString(),conn);
            krokNum = 1;
            foreach (KrokMerania k in ks)
            {
                long y_id = getYID(k.Y, cyklus, aktualneMeranie); //ak neexistuje taky zaznam, vytvori novy a vrati id
                c.Parameters.AddWithValue("@x"+ krokNum.ToString(), k.X);
                c.Parameters.AddWithValue("@y_id" + krokNum.ToString(), y_id);
                c.Parameters.AddWithValue("@sig" + krokNum.ToString(), k.Intensity);
                c.Parameters.AddWithValue("@current" + krokNum.ToString(), k.Current);
                c.Parameters.AddWithValue("@kapillar" + krokNum.ToString(), k.Capillar);
                c.Parameters.AddWithValue("@chamber" + krokNum.ToString(), k.Chamber);
                c.Parameters.AddWithValue("@temperature" + krokNum.ToString(), k.Temperature);
                c.Parameters.AddWithValue("@x_num" + krokNum.ToString(), krokNum);
                krokNum++;
            }
            c.ExecuteNonQuery();
        }
        public int Cycle { get; set; }
        public void addKrok(object obj)
        {
            KrokMerania k=(KrokMerania)obj;
            long y_id = getYID(k.Y, Cycle, aktualneMeranie); //ak neexistuje taky zaznam, vytvori novy a vrati id
            MySqlCommand c = new MySqlCommand("insert into merania (x,y_id,Intensity,Current,Capillar_pressure,Chamber_pressure,Temperature,x_num) " +
                "values(@x,@y_id,@sig,@current,@kapillar,@chamber,@temperature,@x_num);",conn);
            ...
            c.Parameters.AddWithValue("@x_num",k.StepNumber);
        ...
        public void addCyklus(object c)
        {
            addKroky(((CyklusMerania)c).KrokyMerania,((CyklusMerania)c).cisloCyklu);    
        }
